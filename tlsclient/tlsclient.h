...
// Uses WinHTTP for HTTPS connections to swag.live
class TLSClient {
public:
    TLSClient();
    ~TLSClient();

    // Initialize global TLS subsystem (call once at startup)
...
namespace TLSClientHTTP {
    ...
    // HTTP GET with TLS client (returns response body as string)
    std::string HttpGet(const std::wstring& host, const std::wstring& path, const std::wstring& headers = L"");

    // HTTP POST with TLS client (returns response body as string)
